...
| 20 | Rate limiting | `express-rate-limit` was installed and configured in `server.js` — and **commented out**, so nothing was limited at all | two layers: **10 requests per minute per IP** across the seven account endpoints, and **3 codes per 10 minutes per email address** on OTP generation | The OTP endpoints are public, need no token, and send a real email on every call: an open door to exhausting the mail quota or flooding a stranger's inbox. **The old configuration would not have helped even if it had been enabled** — 100 requests per 15 minutes per IP, applied globally, does nothing about many IPs targeting one address. **Two placement decisions matter.** The email token is taken *before* the account lookup, because charging it only for real accounts would make the limiter an oracle for which addresses are registered — reintroducing the enumeration that #9 and #13 removed. And the email limit is deliberately loose, because a limit keyed on the victim's own identifier is itself a way to stop them receiving their code; the IP limit does the volume work. Requires `server.forward-headers-strategy: native` in prod, or `getRemoteAddr()` returns Render's proxy for everyone and the whole world shares one bucket. |
